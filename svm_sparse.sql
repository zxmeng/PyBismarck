...
DROP FUNCTION IF EXISTS sparse_svm_agg(integer[], double precision[], integer, double precision[]) CASCADE;
CREATE FUNCTION sparse_svm_agg(k integer[], v double precision[], y integer, linear_model double precision[])
    RETURNS double precision[]
AS $$
    import numpy as np
...
DROP FUNCTION IF EXISTS sparse_svm_loss(integer[], double precision[], integer, double precision[]) CASCADE;
CREATE FUNCTION sparse_svm_loss(k integer[], v double precision[], y integer, linear_model double precision[])
    RETURNS double precision
AS $$
    import numpy as np
...
DROP FUNCTION IF EXISTS sparse_svm_agg_iteration(text, integer) CASCADE;
...
DECLARE
    ...
    -- loss
    EXECUTE 'SELECT sum(sparse_svm_loss(k, v, label, 
                           (SELECT parms
                            FROM linear_model 
                            WHERE mid = ' || model_id || '))) '
            || 'FROM ' || quote_ident(data_table)
...
DROP FUNCTION IF EXISTS sparse_svm_train_agg(text, integer, integer) CASCADE;
...
DROP FUNCTION IF EXISTS sparse_svm(text, integer, integer, integer, double precision, double precision, boolean) CASCADE;
...
CREATE FUNCTION sparse_svm(
    ...
    ndims integer)
RETURNS VOID AS $$
    SELECT sparse_svm($1, $2, $3, 20, 1e-2, 5e-1, 't');
